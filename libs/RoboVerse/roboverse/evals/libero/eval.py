...
from roboverse.unifiers.image_unifier import (image_unifier_transform,
                                              remove_keys)
from roboverse.evals.libero.eval_run.eval_run import eval_run
...
def get_evaluation_tasks(task_suite_name=None, task_name=None):
    ...
    # for evaluating on a particular task suite and all the tasks in the task suite 
    ...
    for task_suite_name, tasks in tasks_to_evaluate.items():
        ...
        for task_name in tasks:
            ...
            for i, init_state in tqdm(enumerate(init_states)):
                ...
                """
                General VLA-0 implementation
                """
                success, frames = _eval_.get_final_actions_and_frame()
                """
                VLA-0 implementation with probabilistically scaled particle based MCM rollouts to get final actions and frames. or PCM """
                # success, frames = _eval_.get_final_actions_and_frame_prob_pbmcm(
                #     inference_budget= 4,
                #     temperature= 0.1
                # )
                # 
                """
                VLA-0 implementation with MCTS action search in the action chunking 
                """
                # success, frames = _eval_.get_final_actions_and_frame_mcts(
                #     n_simulations = 8,
                #     # n_simulations=4,
                #     rollout_depth = 12,
                #     c_puct= 1.25
...
#         # frame skip at the beginning of the episode to let the simulator settle down.
#         # During frame skip, we take a dummy action and don't feed the obs to the model. 
#         # This is because the initial few frames can be very noisy and can cause the model to output very wrong actions which can cause the simulator to go into a bad state and never recover from it.
